...
class Game {
    ...
    move(extract) {
        ...
        for(let i = 0; i < 4; i++){
            ...
            this.score += score;
            for(let j = 0; j < 4; j++)if (extract === this.moveLeft || extract === this.moveRight) newBoard[i][j] = result[j] || 0;
            else newBoard[j][i] = result[j] || 0;
        }
    ...
    merge(line) {
        ...
        if (!changed) changed = result.some((v, i)=>v !== line[i]);
        return {
            result,
            score,
            changed
